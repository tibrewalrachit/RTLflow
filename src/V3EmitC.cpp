...
public:
    ...
    void emitIntFuncDecls(AstNodeModule* modp, bool methodFuncs) {
        ...
        if (methodFuncs && modp->isTop() && v3Global.opt.mtasks()) {
            ...
            for (const V3GraphVertex* vxp = depGraphp->verticesBeginp(); vxp;
                ...
                // RTLflow
                // puts("(void* symtab, CData* _csignals, SData* _ssignals, IData* _isignals,
                // QData* "
    ...
    // TODO: use taskflow to stop/finish operations
    ...
    virtual void visit(AstVarRef* nodep) override {
        ...
        if (varp->isCuda()) {
            ...
            if (m_isGpu) {
                puts("(blockDim.x * blockIdx.x + threadIdx.x)");
            } else {
                puts("i");
            }
            ...
            if (adtypep != nullptr) {
                ...
                if (adtypep->subDTypep()->isWide()) {
                    puts(" * " + cvtToStr(adtypep->subDTypep()->widthWords()));
                }
            } else if (varp->isWide()) {
                puts(" * " + cvtToStr(varp->widthWords()));
            }
            puts(" + ");
...
class EmitCImp final : public EmitCStmts {
    ...
    V3OutCFile* newOutCFile(bool slow, bool source, int filenum = 0) {
        ...
        if (v3Global.opt.lintOnly()) {
            ...
            string filename = filenameNoExt + (source ? ".cu" : ".h");
            newCFile(filename, slow, source);
            ofp = new V3OutCFile(filename);
        }
    ...
    virtual void visit(AstCFunc* nodep) override {
        ...
        auto prev_isGpu = m_isGpu;
        m_isGpu = nodep->device();
...
void EmitCStmts::emitVarDecl(const AstVar* nodep, const string& prefixIfImp) {
    ...
    if (nodep->isCuda() && nodep->isIO()) {
        ...
        ////auto type_mem = type_size(adtypep->subDTypep());
        ////type = "RfUnpacked<" + std::get<0>(type_mem);
        ////type += ", " + cvtToStr(adtypep->declRange().elements());
...
void EmitCImp::emitCtorImp(AstNodeModule* modp) {
    ...
    if (VN_IS(modp, Class)) {
        ...
        puts(prefixNameProtect(modp) + "::" + prefixNameProtect(modp)
             + "(CData* _csignals, SData* _ssignals, IData* _isignals, QData* _qsignals, const "
               "char* _vcname__)\n");
        puts("    : VerilatedModule(_vcname__)\n");
    }
    emitVarCtors(&first);
    ...
    if (modp->isTop()) {
        puts("_rtlflow.initialize(vlSymsp);\n\n");
    ...
    if (modp->isTop()) {
        puts(protect("_ctor_var_reset")
             + "(this, _rtlflow._csignals, _rtlflow._ssignals, _rtlflow._isignals, "
               "_rtlflow._qsignals);\n");
    } else {
        puts(protect("_ctor_var_reset") + "(this, _csignals, _ssignals, _isignals, _qsignals);\n");
    }
    puts("reset_cell_counter++;\n");
...
void EmitCImp::emitCellCtors(AstNodeModule* modp) {
    if (modp->isTop()) {
        ...
        puts(EmitCBaseVisitor::symClassVar() + " = __VlSymsp = new " + symClassName() + "("
             + (optSystemC() ? "nullptr" : "_vcontextp__")
             + ", this, _rtlflow._csignals, _rtlflow._ssignals, _rtlflow._isignals, "
               "_rtlflow._qsignals, name());\n");
        puts(EmitCBaseVisitor::symTopAssign() + "\n");
    }
    for (AstNode* nodep = modp->stmtsp(); nodep; nodep = nodep->nextp()) {
...
void EmitCImp::emitInt(AstNodeModule* modp) {
    ...
    // cell counters
    puts("static size_t reset_cell_counter;");
...
class cudaCheck final : AstNVisitor {
    ...
    struct equal {
        bool operator()(const AstVarRef* lhs, const AstVarRef* rhs) const {
            return (lhs->same(rhs));
        }
    };

    struct hash {
        size_t operator()(const AstVarRef* avr) const {
            return std::hash<std::string>()(avr->hiernameToProt() + avr->hiernameToUnprot()
                                            + avr->varp()->name());
        }
    };

    std::unordered_set<AstVarRef*, hash, equal> m_lookup;
    ...
    virtual void visit(AstVarRef* nodep) override {
        ...
        AstNodeDType* dtypep{nullptr};
        if (varp->isCuda()) {
            if (m_lookup.find(nodep) != m_lookup.end()) { return; }

            m_lookup.insert(nodep);
            m_counter++;
            if (m_counter >= 10000) {
                // split and create another file
                create_file();
                m_counter = 0;
            }
            ...
            m_ofp->puts("RF_ASSERT(");

            if (dtypep->widthMin() <= 8) {
                m_ofp->puts("_csignals");
            } else if (dtypep->widthMin() <= 16) {
                m_ofp->puts("_ssignals");
            } else if (dtypep->isQuad()) {
                m_ofp->puts("_qsignals");
            } else {
                // IData, WData
                m_ofp->puts("_isignals");
            }

            m_ofp->puts("[");

            // m_ofp->puts("(blockDim.x * blockIdx.x + threadIdx.x)");
            // m_ofp->puts("(0)");

            // if (adtypep != nullptr) {
            // m_ofp->puts(" * " + cvtToStr(adtypep->arrayUnpackedElements()));
            // if (adtypep->subDTypep()->isWide()) {
            // m_ofp->puts(" * " + cvtToStr(adtypep->subDTypep()->widthWords()));
            //}
            //} else if (varp->isWide()) {
            // m_ofp->puts(" * " + cvtToStr(varp->widthWords()));
            //}
            // m_ofp->puts(" + ");
            ...
            m_ofp->puts(" == " + nodep->hiernameProtect() + varp->nameProtect() + ", \"\");\n");
        }
    }
...
public:
    ...
    void create_file() {
        if (m_num_files != 0) { m_ofp->puts("\n}"); }
        ++m_num_files;
        m_ofp = new V3OutCFile(v3Global.opt.makeDir() + "/check" + cvtToStr(m_num_files) + ".cu");

        string topClassName = v3Global.opt.prefix();
        m_ofp->puts("#include <iostream>\n");
        m_ofp->puts("#include \"rtlflow.h\"\n");
        m_ofp->puts("#include \"" + topClassName + ".h\"\n");
        m_ofp->puts("#include \"../verilator/" + topClassName + ".h\"\n");
        m_ofp->puts("#include \"../verilator/" + topClassName + "__Syms.h\"\n");

        // micros
        m_ofp->puts("#ifdef NDEBUG\n");
        m_ofp->puts("#define RF_ASSERT(condition, message) 0\n");
        m_ofp->puts("#else\n");
        m_ofp->puts("#define RF_ASSERT(condition, message)\\\n");
        m_ofp->puts("  (!(condition)) ?\\\n");
        m_ofp->puts("    (std::cerr << \"Assertion failed: (\" << #condition << \"), \"\\\n");
        m_ofp->puts("    << \"function \" << __FUNCTION__\\\n");
        m_ofp->puts("    << \", file \"   << __FILE__\\\n");
        m_ofp->puts("    << \", line \"   << __LINE__ << \".\"\\\n");
        m_ofp->puts("    << \"\\n\"       << message  << \"\\n\", 0) : 1\n");
        m_ofp->puts("#endif\n");

        m_ofp->puts("void check" + cvtToStr(m_num_files) + "(" + topClassName + "* vlTOPp) {\n");
        m_ofp->puts(topClassName + "__Syms* vlSymsp = vlTOPp->__VlSymsp;\n");
        m_ofp->puts("CData* _csignals = RF::" + topClassName + "::_rtlflow._csignals;\n");
        m_ofp->puts("SData* _ssignals = RF::" + topClassName + "::_rtlflow._ssignals;\n");
        m_ofp->puts("QData* _qsignals = RF::" + topClassName + "::_rtlflow._qsignals;\n");
        m_ofp->puts("IData* _isignals = RF::" + topClassName + "::_rtlflow._isignals;\n");
    }
};
...
class cudaMemAssign final : public AstNVisitor {
    ...
    using Key = std::pair<AstScope*, AstVar*>;
...
public:
    ...
    void assignLoc(AstVarRef* nodep) {
        ...
        if (varp->isCuda()) {
            ...
            auto search = m_memLocMap.find({nodep->scopep(), varp});
...
void V3EmitC::emitRTLflowImp() {
    ...
    of.puts("namespace RF {\n");
    of.puts("inline\n");
    of.puts("cudaError_t checkCuda(cudaError_t result) {\n");
    of.puts("if (result != cudaSuccess) {\n");
    of.puts("using namespace std::literals::string_literals;\n");
    of.puts("throw std::runtime_error(\"CUDA Runtime Error: \"s + cudaGetErrorString(result));\n");
    of.puts("}\n");
    of.puts("return result;\n");
    of.puts("}\n\n");
    of.puts("__global__ void _eval_settle(" + topClassName
